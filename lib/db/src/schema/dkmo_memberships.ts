...
export const dkmoMembershipsTable = pgTable("dkmo_memberships", {
  id: uuid("id").primaryKey().defaultRandom(),
  dkmoNumber: text("dkmo_number").notNull().unique(),
  ...
  familyInSaudi: text("family_in_saudi").notNull().default(""),
...
export const dkmoMembershipDependentsTable = pgTable("dkmo_membership_dependents", {
  id: uuid("id").primaryKey().defaultRandom(),
  dkmoMembershipId: uuid("dkmo_membership_id").notNull().references(() => dkmoMembershipsTable.id, { onDelete: "cascade" }),
  fullName: text("full_name").notNull(),
