...
const STATUS_CONFIG: Record<AppStatus, {
  ...
  approved: {
    label: "Approved",
    description: "Congratulations! Your FRF membership application has been approved. DKMO will be in touch shortly.",
  ...
  completed: {
    label: "Completed",
    description: "Your FRF membership is active. Welcome to the DKMO Family Relief Fund family!",
    icon: <CheckCircle className="h-10 w-10 text-green-600 dark:text-green-400" />,
    cardClass: "border-green-200 dark:border-green-900/50 bg-green-50/50 dark:bg-green-950/10",
    badgeClass: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300",
    dot: "bg-green-500",
...
export default function FrfTrackPage() {
  const [, setLocation] = useLocation();
  const [query, setQuery] = useState("");
  const [queryType, setQueryType] = useState<"frfNumber" | "mobile">("frfNumber");
  ...
  async function handleSearch(e?: FormEvent) {
    ...
    setResults(null);
    try {
      const param = queryType === "frfNumber" ? `frfNumber=${encodeURIComponent(q)}` : `mobile=${encodeURIComponent(q)}`;
      const res = await fetch(`${basePath}/api/frf/memberships/track?${param}`);
      if (!res.ok) {
      ...
      if (!data.length) {
        setError("No application found. Please check your FRF number or mobile number and try again.");
      } else {
        setResults(data);
      }
    } catch (err) {
    ...
    <div className="min-h-[100dvh] bg-gradient-to-br from-green-50 via-white to-orange-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <div className="border-b border-green-100 dark:border-slate-800 bg-white/60 dark:bg-slate-950/60 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          <button
            onClick={() => setLocation(`${basePath}/frf-apply`)}
          ...
          <div>
            ...
            <p className="text-xs text-green-700/60 dark:text-slate-400">DKMO — Family Relief Fund</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10 space-y-6">
        ...
        <Card className="rounded-2xl border-green-100 dark:border-slate-800 dark:bg-slate-900 shadow-sm">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-slate-500 dark:text-slate-400">
              Enter your FRF application number or the mobile number you registered with.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="flex rounded-xl border border-green-200 dark:border-slate-700 overflow-hidden text-sm">
                <button
                  type="button"
                  onClick={() => { setQueryType("frfNumber"); setResults(null); setError(null); }}
                  className={`flex-1 py-2 font-medium transition-colors ${queryType === "frfNumber" ? "bg-green-800 text-white" : "bg-transparent text-green-700 dark:text-slate-300 hover:bg-green-50 dark:hover:bg-slate-800"}`}
                >
                  FRF Number
                </button>
                <button
              ...
              <div className="space-y-1.5">
                <Label htmlFor="track-query" className="text-sm text-green-900 dark:text-slate-300">
                  {queryType === "frfNumber" ? "FRF Application Number" : "Registered Mobile Number"}
                </Label>
                <Input
                  id="track-query"
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder={queryType === "frfNumber" ? "e.g. FRF-2024-0001" : "e.g. +966501234567"}
        ...
        {results && results.map((r) => {
          ...
          return (
            <Card key={r.frfNumber} className={`rounded-2xl shadow-sm border-2 ${cfg.cardClass}`}>
              <CardContent className="pt-6 space-y-5">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500 dark:text-slate-400">FRF Number</span>
                    <span className="font-mono font-semibold text-green-800 dark:text-green-300">{r.frfNumber}</span>
                  </div>
                  <div className="flex justify-between">
