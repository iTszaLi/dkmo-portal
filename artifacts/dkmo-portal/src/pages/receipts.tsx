...
import {
  ...
  QrCode, CheckCircle, AlertTriangle, ShieldCheck, ScanLine, ReceiptText,
} from "lucide-react";
...
async function buildReceiptPdf(
  ...
  if (receiptNumber) {
    try {
      const verifyUrl = `${window.location.origin}${basePath}/receipts?verify=${encodeURIComponent(receiptNumber)}`;
...
export default function ReceiptsHub() {
  ...
  const [activeTab, setActiveTab] = useState<"generate" | "payments" | "records" | "verify">(() => {
    const t = new URLSearchParams(window.location.search).get("tab");
    return t === "generate" || t === "payments" || t === "records" || t === "verify" ? t : "generate";
  });
  ...
  const clearMonthFilter = () => setLocation("/receipts");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-emerald-950 dark:text-emerald-100">Receipts</h1>
          <p className="text-emerald-700/80 dark:text-slate-400">Generate, search, print, download and verify receipts</p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
        <TabsList className="border border-emerald-100 dark:border-slate-800 bg-emerald-50/50 dark:bg-slate-900">
          <TabsTrigger value="generate" className="data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  ...
                  Receipts created in the <strong>Generate</strong> tab are automatically logged here.
                </p>
              </div>
            </CardContent>
          </Card>
