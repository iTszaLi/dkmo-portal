...
import { Receipt, Download, Printer, RotateCcw, FolderOpen, Save, Wand2, AlertCircle } from "lucide-react";
...
async function generateReceiptPdf(data: FormValues, basePath: string, withQr = true): Promise<jsPDF> {
  ...
  if (logoDataUrl) doc.addImage(logoDataUrl, "PNG", 4, 3, 22, 22);
  ...
  const bW = 26, bH = 9, startX = 24, gap = 2;
  boxes.forEach((b, i) => {
    const bx = startX + i * (bW + gap);
    const by = y - 2;
    doc.setFillColor(255, 255, 255);
    if (b.checked) {
      doc.setDrawColor(0, 120, 0); doc.setLineWidth(0.6);
    } else {
    ...
    if (b.checked) {
      ...
      doc.setDrawColor(0, 130, 0); doc.setLineWidth(1.0);
    ...
    lines.forEach((line, li) => { doc.text(line, bx + bW / 2, by + bH + 3.5 + li * 4, { align: "center" }); });
  });
  ...
  if (withQr && data.receiptNumber) {
    try {
      const verifyUrl = `${window.location.origin}${basePath}/print-receipts?verify=${encodeURIComponent(data.receiptNumber)}`;
      const qrDataUrl = await QRCode.toDataURL(verifyUrl, { width: 60, margin: 1 });
      doc.addImage(qrDataUrl, "PNG", W - 26, 4, 20, 20);
      doc.setFont("helvetica", "normal"); doc.setFontSize(5); doc.setTextColor(80, 80, 80);
      doc.text("Scan to verify", W - 16, 25.5, { align: "center" });
    } catch { /* QR optional */ }
  }

  y += 22;
  doc.setDrawColor(...maroon); doc.setLineWidth(0.8);
  doc.line(4, y, W - 4, y);

  y += 6;
  doc.setTextColor(...maroon); doc.setFont("helvetica", "bold"); doc.setFontSize(9.5);
  doc.text("COMMITTED TO THE COMMUNITY", W / 2, y, { align: "center" });

  y += 5;
  const leftItems = ["Frf Scheme (Family Relief Fund)", "Medical Aid", "Free Air Ticket To Stranded NRI's", "General Relief Fund"];
  const rightItems = ["Representative In India For Health & Govt Scheme Utilization", "Emergency Response Scheme.", "Dkmo Job Bureau", "Loan Scheme For Members"];
  doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(30, 30, 30);
  leftItems.forEach((s, i) => { doc.text(`✦  ${s}`, 8, y + i * 6); });
  rightItems.forEach((s, i) => { doc.text(`✦  ${s}`, W / 2 + 4, y + i * 6); });

  const sigY = y + 25;
  doc.setDrawColor(...gray); doc.setLineWidth(0.3); doc.line(W - 56, sigY, W - 6, sigY);
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: "8px", marginBottom: "10px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", minWidth: "170px" }}>
          ...
          <span className={dotLine} style={{ borderBottom: "1px dotted #777", flex: 1, marginBottom: "1px" }}>{data.receiptDate}</span>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", minWidth: "140px" }}>
          ...
          <span style={{ borderBottom: "1px dotted #777", flex: 1, minWidth: "60px", marginBottom: "1px", paddingLeft: "4px" }}>{data.receiptNumber}</span>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", flex: 1 }}>
          <span style={{ fontWeight: "bold", whiteSpace: "nowrap" }}>Jamath Name:</span>
          <span style={{ borderBottom: "1px dotted #777", flex: 1, marginBottom: "1px", paddingLeft: "4px" }}>{data.jamathName}</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: "8px", marginBottom: "10px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", flex: 1.5 }}>
          ...
          <span style={{ borderBottom: "1px dotted #777", flex: 1, marginBottom: "1px", paddingLeft: "4px" }}>{data.memberName}</span>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", flex: 1 }}>
          <span style={{ fontWeight: "bold", whiteSpace: "nowrap" }}>DKMO ID # :</span>
          <span style={{ borderBottom: "1px dotted #777", flex: 1, marginBottom: "1px", paddingLeft: "4px" }}>{data.dkmoId}</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: "8px", marginBottom: "10px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", flex: 1.5 }}>
          ...
          <span style={{ borderBottom: "1px dotted #777", flex: 1, marginBottom: "1px", paddingLeft: "4px" }}>{data.mobileNumber}</span>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", flex: 1 }}>
          <span style={{ fontWeight: "bold", whiteSpace: "nowrap" }}>Whatsapp # :</span>
          <span style={{ borderBottom: "1px dotted #777", flex: 1, marginBottom: "1px", paddingLeft: "4px" }}>{data.whatsappNumber}</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: "8px", marginBottom: "6px" }}>
        ...
        <div style={{ display: "flex", gap: "5px", flexWrap: "nowrap", flex: 1 }}>
          {boxes.map((b, i) => (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: "72px" }}>
              <div style={{ width: "68px", height: "26px", border: b.checked ? "1.5px solid green" : "1.5px solid #444", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "bold", color: b.checked ? "green" : "#000" }}>
                {b.checked ? "✓" : ""}
              </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px", flexShrink: 0, paddingTop: "2px" }}>
          ...
          <div style={{ width: "80px", height: "26px", border: "1.5px solid #444", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold", fontSize: "13px" }}>
...
export default function ReceiptsPage() {
  ...
  const createRecordMutation = useCreateReceiptRecord({
    mutation: {
      onSuccess: () => {
        setSaved(true);
        toast({ title: "Receipt record saved" });
      },
      onError: (err) => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      mobileNumber: "",
      whatsappNumber: "",
      paymentTypes: { lifeMembership: false, frfCase: "", voluntaryYearly: false, donation: false, loanRecovery: false, others: false },
      amount: 0,
    },
  });
  ...
  const autoFillReceiptNumber = useCallback(async () => {
    setAutoFilling(true);
    try {
      const resp = await fetch("/api/receipts/next-number");
      if (!resp.ok) throw new Error("Failed");
      const { nextNumber } = await resp.json() as { nextNumber: string };
      form.setValue("receiptNumber", nextNumber, { shouldValidate: true });
      toast({ title: `Auto-filled: ${nextNumber}` });
    } catch {
      toast({ title: "Could not fetch next receipt number", variant: "destructive" });
    } finally {
      setAutoFilling(false);
    }
  }, [form, toast]);

  // Check for duplicate receipt number when it changes
  useEffect(() => {
    if (!watchedReceiptNumber || watchedReceiptNumber.length < 5) {
  ...
  }, [watchedReceiptNumber]);

  const onDownload = useCallback(
    form.handleSubmit(async (values) => {
      try {
        const doc = await generateReceiptPdf(values, getBasePath(), true);
      ...
      <div className="space-y-6 p-6 max-w-5xl mx-auto">
        ...
        <Form {...form}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-5 space-y-5">
                ...
                <FormItem>
                  ...
                  <div className="flex gap-2">
                    <FormControl>
                      <Input
                        ...
                        className={duplicateWarning ? "border-amber-400 dark:border-amber-500" : ""}
                      />
                    </FormControl>
                    <Button
                      type="button"
                      variant="outline"
                      size="icon"
                      title="Auto-fill next receipt number"
                      onClick={autoFillReceiptNumber}
                      disabled={autoFilling}
                      className="shrink-0 border-green-200 dark:border-slate-700 text-green-700 dark:text-green-400 hover:bg-green-50 dark:hover:bg-slate-800"
                    >
                  ...
                  {duplicateWarning && (
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <p className="text-xs text-amber-600 dark:text-amber-400">This receipt number already exists in records</p>
                    </div>
                  )}
                  <FormMessage />
                </FormItem>
              ...
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  <FormField key={item.name} control={form.control} name={item.name} render={({ field }) => (
                    <label className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 cursor-pointer select-none transition-colors text-sm font-medium
                      ...
                      <input type="checkbox" className="sr-only" checked={field.value as boolean} onChange={field.onChange} />
                ...
                <FormField control={form.control} name="paymentTypes.frfCase" render={({ field }) => (
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 transition-colors ${field.value ? "border-green-600 bg-green-50 dark:bg-green-950/30" : "border-slate-200 dark:border-slate-700"}`}>
                    ...
                    <Input className="h-7 w-28 text-sm" placeholder="Case #" value={field.value} onChange={(e) => field.onChange(e.target.value)} />
                  </div>
                )} />
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-100 dark:border-slate-700">
              ...
              <Button type="button" variant="ghost" className="gap-2 text-slate-500" onClick={() => { form.reset(); setSaved(false); setDuplicateWarning(false); }}>
            ...
            {duplicateWarning && (
              <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-sm">
                <AlertCircle className="h-4 w-4 shrink-0" />
                <span><strong>Warning:</strong> Receipt number already exists in the system. Consider using a different number or click the ✦ button to auto-generate the next sequential number.</span>
              </div>
            )}
          </div>
        </Form>
        ...
        <div>
          <p className="text-sm font-semibold text-slate-600 dark:text-slate-400 mb-3 flex items-center gap-2">
            ...
            <span className="font-normal text-slate-400 dark:text-slate-500 text-xs">(PDF includes a QR verification code)</span>
          </p>
